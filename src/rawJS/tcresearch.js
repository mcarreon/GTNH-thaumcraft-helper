$(function () {
  ...
  function push_addons(aspects, combinations) {
    ...
    $.each(addon_dictionary, function (key, addon_info) {
      ...
      $.each(addon_info["combinations"], function (
        combination_name,
        combination
      ) {
  ...
  function toggle(obj) {
    $(obj)
      ...
      .attr("src", function (i, orig) {
        return orig.indexOf("color") < 0
          ? orig.replace(/mono/, "color")
          : orig.replace(/color/, "mono");
      });
  ...
  function toggle_addons(aspect_list) {
    aspect_list.forEach(function (e) {
      var obj = $("#" + e);
      obj.find("img").attr("src", function (i, orig) {
  ...
  $("body").on("mouseenter", ".aspectlist .aspect", function () {
    ...
    if (
      ...
      $("#combination_box #left").html(
        '<img src="aspects/color/' +
          translate[combination[0]] +
          '.png" /><div class="name">' +
          formatAspectName(translate[combination[0]]) +
          '</div><div class="desc">' +
          combination[0] +
          "</div>"
      );
      $("#combination_box #right").html(
        '<img src="aspects/color/' +
          translate[combination[1]] +
          '.png" /><div class="name">' +
          formatAspectName(translate[combination[1]]) +
          '</div><div class="desc">' +
          combination[1] +
          "</div>"
      );
      $("#combination_box #equals").html(
        '<img src="aspects/color/' +
          translate[aspect] +
          '.png" /><div class="name">' +
          formatAspectName(translate[aspect]) +
          '</div><div class="desc">' +
          aspect +
          "</div>"
      );
      $(this).mousemove(function (e) {
        $("#combination_box")
          .css({ left: e.pageX + 10, top: e.pageY - 100 })
          .show();
      });
    } else {
  ...
  function reset_aspects() {
    ...
    function format(d) {
      ...
      return (
        '<div class="aspect" id="' +
        aspect +
        '"><img style="margin: 4px 5px 0 0" src="aspects/color/' +
        translate[aspect] +
        '.png" /><div>' +
        formatAspectName(translate[aspect]) +
        '</div><div class="desc">' +
    ...
    $("#toSel,#fromSel").select2({
      ...
      matcher: function (search, text) {
        return (
          text.toUpperCase().indexOf(search.toUpperCase()) >= 0 ||
  ...
  function run() {
    ...
    var title =
      formatAspectName(translate[fromSel]) +
      " &rarr; " +
      formatAspectName(translate[toSel]);
    var step_count = 0;
    var aspect_count = {};
    ...
    path.forEach(function (e) {
      ...
      $("#" + id).append(
        '<li class="aspect_result aspect" id="' +
          e +
          '"><img src="aspects/color/' +
          translate[e] +
          '.png" /><div>' +
          formatAspectName(translate[e]) +
          '</div><div class="desc">' +
          e +
          "</div></li><li>↓</li>"
      );
      ...
      if (value > 0) {
        used = $(used).append(
          '<li title="' +
            ...
            ": " +
            value +
            '" style="background-image:url(\'aspects/color/' +
            translate[aspect] +
            ".png')\">" +
            value +
            "</li>"
